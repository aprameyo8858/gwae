...
class GromovWassersteinAutoEncoder(VAE):
    ...
    def step_batch(self, batch: tuple[torch.Tensor, torch.Tensor], optimizers=None, progress=None, training=False):
        ...
        #loss_gw = rasgw_gpu(x2s_.to('cuda'), zs_.to('cuda') , 'cuda',L=10, nproj=5000,tolog=False,P=None)
        loss_gw = iwrasgw_gpu(x2s_.to('cuda'), zs_.to('cuda') , 'cuda',L=10, nproj=5000,tolog=False,P=None)
