...
def iwrasgw_gpu(xs, xt, device, nproj=200, tolog=False, P=None):
    ...
    References
    ----------
    .. [1] Vayer Titouan, Chapel Laetitia, Flamary Rémi, Tavenard Romain
          and Courty Nicolas
    ...
    if tolog: 
        st = time.time()
        xsp, xtp, P = sink_(xs, xt, device, nproj, P)
        ed = time.time()   
    ...
    # Now apply the weighted distance to the result.
    weighted_d = weighted_distance(d)  # Apply the weighted sum with softmax

    if tolog:
        log['weighted_distance'] = weighted_d

    return weighted_d, log if tolog else weighted_d
...
def gromov_1d(xs, xt, tolog=False): 
    ...
    if tolog:
        ...
        xt_desc, i_t = torch.sort(xt, dim=0, descending=True)  # sort decrease
...
def sink_(xs, xt, device, nproj=None, P=None, kappa=50): 
    ...
    if P is None:  # Compute projection matrix if not passed
        ...
        theta = torch.nan_to_num(theta, nan=0.0)  # Replace NaNs with 0
        
        if torch.isnan(theta).any():
            print("NaN values detected in loc.")
        
        # Compute weights based on the projection directions (P matrix)
        ps = PowerSpherical(
            loc=theta,
        ...
        theta = ps.rsample()
        P = theta.T  # Transpose to match dimensions

    try:
        # Project source and target samples onto the projection matrix
        xsp = torch.matmul(xs2, P.to(device))  # Project source samples
        xtp = torch.matmul(xt2, P.to(device))  # Project target samples
    except RuntimeError as error:
        print(f"Error during projection: {error}")
        raise BadShapeError
    
    return xsp, xtp, P  # Return the projections and the distances for each direction
